...
const wchar_t *exts[] = {L".tnz", L".tlv", L".pli"};
const wchar_t *progId = L"OpenToonz.ToonzPreview";
const int EXT_COUNT   = 3;
  ...
  std::call_once(qtInitFlag, []() {
    if (QCoreApplication::instance()) {
      ...
      if (GetModuleFileNameW(g_hInst, dllPath, MAX_PATH)) {
        ...
        // **[Keep]** Set the DLL path as the first item in
        // QCoreApplication::libraryPaths This allows Qt to find DLLs and
        // plugins in the DLL directory.
        QString exePath   = QString::fromStdWString(dllPath);
        QStringList paths = QCoreApplication::libraryPaths();
        paths.removeOne(exePath);
        paths.prepend(exePath);
        QCoreApplication::setLibraryPaths(paths);
        DebugLog("DLL path prepended to QCoreApplication::libraryPaths.");

        // Set the general plugin path (contains audio, imageformats, styles,
        // etc.)
        qputenv("QT_PLUGIN_PATH", dllDirNarrow.c_str());
        DebugLog("Set QT_PLUGIN_PATH to: " + dllDirNarrow);

        // Set the platform plugin path (contains qwindows.dll)
        std::wstring platformsPathW     = std::wstring(dllDir) + L"\\platforms";
        std::string platformsPathNarrow = WideToNarrow(platformsPathW);

        // Check if the platform plugins directory exists to ensure correct
        // setting
        if (PathIsDirectoryW(platformsPathW.c_str())) {
          qputenv("QT_QPA_PLATFORM_PLUGIN_PATH", platformsPathNarrow.c_str());
          DebugLog("Set QT_QPA_PLATFORM_PLUGIN_PATH to: " +
                   platformsPathNarrow);
        } else {
          DebugLog(
              "WARNING: Platform plugins directory (\\platforms) not found.");
        }
      ...
      // Set software rendering backend
      qputenv("QT_QUICK_BACKEND", "software");

      DebugLog("Environment variables set, creating QCoreApplication...");

      try {
        static int argc     = 1;
        static char *argv[] = {(char *)"toonzpreview_host", nullptr};
        // Use QCoreApplication constructor
        ...
        // Check if plugins are loaded (for debugging)
        QStringList pluginPaths = QCoreApplication::libraryPaths();
        DebugLog("Qt library paths:");
      ...
      DebugLog("Qt application initialization completed successfully");
    }
  });
}
...
HRESULT RegisterRegistryKeys() {
  ...
  for (int i = 0; i < EXT_COUNT; i++) {
    // Extension Key
    std::wstring extKey = std::wstring(L"SOFTWARE\\Classes\\") + exts[i];
    if (RegCreateKeyExW(HKEY_LOCAL_MACHINE, extKey.c_str(), 0, NULL,
    ...
    // ShellEx (The Thumbnail Handler)
    // Important: Key Name must be the Standard IThumbnailProvider IID
    ...
    if (RegCreateKeyExW(HKEY_LOCAL_MACHINE, shellExKey.c_str(), 0, NULL,
                        REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey,
                        NULL) == ERROR_SUCCESS) {
      // The VALUE must be OUR CLSID
      RegSetValueExW(hKey, NULL, 0, REG_SZ, (BYTE *)SZ_CLSID_TOONZPREVIEW,
                     (wcslen(SZ_CLSID_TOONZPREVIEW) + 1) * sizeof(WCHAR));
      RegCloseKey(hKey);
      DebugLog("Registered thumbnail handler for " + WideToNarrow(exts[i]));
    }
  }

  // 3. Register System File Associations (Important!)
  for (int i = 0; i < EXT_COUNT; i++) {
...
STDAPI DllUnregisterServer() {
  ...
  // Critical Step: Clear the DisableProcessIsolation value before cleaning up
  // the CLSID key
  ...
  for (int i = 0; i < EXT_COUNT; i++) {
    ...
    // SystemFileAssociations cleanup: OK
    std::wstring sysKeyPath =
        std::wstring(L"SOFTWARE\\Classes\\SystemFileAssociations\\") + exts[i] +
        L"\\ShellEx\\" + SZ_ITHUMBNAILPROVIDER_IID;
    RegDeleteKeyW(HKEY_LOCAL_MACHINE, sysKeyPath.c_str());

    // --- Suggested additional cleanup for ProgID and Extension Key ---
    // (Note: Deleting ProgID might affect other associations, proceed with
    // caution)
    std::wstring progIDPath = L"SOFTWARE\\Classes\\" + std::wstring(progId);
    RegDeleteKeyW(HKEY_LOCAL_MACHINE, progIDPath.c_str());

    // 4. **Corrected Extension Key Cleanup (if needed)**
    //    First build the path, then get the pointer
    std::wstring extPath = L"SOFTWARE\\Classes\\" + std::wstring(exts[i]);
    RegDeleteKeyW(HKEY_LOCAL_MACHINE, extPath.c_str());
  }

  // 5. Clean up the DefaultIcon registry key for .tpl file type
  std::wstring tplIconKey = L"SOFTWARE\\Classes\\.tpl\\DefaultIcon";
  RegDeleteKeyW(HKEY_LOCAL_MACHINE, tplIconKey.c_str());
  DebugLog("Cleaned up DefaultIcon for .tpl");
...
IFACEMETHODIMP CToonzThumbnailProvider::GetThumbnail(UINT cx, HBITMAP *phbmp,
  ...
  *pdwAlpha = WTSAT_UNKNOWN;
  ...
  try {
    ...
    } else if (type == "tlv" || type == "tzi" || type == "png" ||
               type == "tif" || type == "jpg") {
      DebugLog("GetThumbnail: Generating Raster Level (TLV/Image) icon");
      ras =
          IconGenerator::generateRasterFileIcon(path, size, TFrameId::NO_FRAME);
    } else if (type == "tpl") {
      DebugLog(
