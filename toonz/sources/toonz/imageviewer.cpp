...
extern ToggleCommandHandler framesToggle;
...
void ImageViewer::paintGL() {
  ...
  if (framesToggle.getStatus() && !m_isColorModel) {
...
void ImageViewer::drawSafeArea(TPointD centerD, TRect bounds) {
  glPushMatrix();
  glLoadIdentity();
  ...
  if (sl) {
    ...
    TAffine finalAff  = m_viewAff * offsetAff;
