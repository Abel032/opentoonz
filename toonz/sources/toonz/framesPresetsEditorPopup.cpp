...
FramesPresetsEditorPopup::FramesPresetsEditorPopup(QWidget* parent)
  ...
  setModal(false);
  connect(this, &QObject::destroyed, []() { FramesPresetsEditorPopup::s_instance = nullptr; });
  ...
  if (m_oldFramesName.endsWith(ViewerDraw::RELOAD_TAG)) {
    m_oldFramesName.chop(QString(ViewerDraw::RELOAD_TAG).length());
  }
  m_iniFilePath = ViewerDraw::getFramesIniPath();
  loadIniFile();
  m_initialPresets = m_presets;
...
void FramesPresetsEditorPopup::setSceneFramesPresetName(const QString& name,
  ...
  QString currentFramesName = sprop->getFramesPresetName();
  sprop->setFramesPresetName(name + ViewerDraw::RELOAD_TAG);
  TApp::instance()->getCurrentScene()->notifySceneChanged(!preview);
}
...
void FramesPresetsEditorPopup::triggerPreview() {
  ...
  if (presetIdx >= 0 && presetIdx < m_presets.size()) {
    nameToApply     = m_presets.at(presetIdx).m_name;
    m_previewActive = true;
  } else {
    nameToApply     = m_oldFramesName;
    m_previewActive = false;
  }

  setSceneFramesPresetName(nameToApply, true);
}
...
void FramesPresetsEditorPopup::accept() {
  ...
  int presetIdx = m_presetList->currentRow();
  if (presetIdx >= 0 && presetIdx < m_presets.size()) {
    setSceneFramesPresetName(m_presets.at(presetIdx).m_name, false);
  }
...
void FramesPresetsEditorPopup::reject() {
  m_presets = m_initialPresets;

  saveIniFile();

  setSceneFramesPresetName(m_oldFramesName, true);
