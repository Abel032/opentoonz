...
void ViewerDraw::getFramesPreset(QList<QList<double>> &_sizeList,
  ...
  bool forceReload =
      currentFramesPresetName.endsWith(RELOAD_TAG);  // defined in viewerdraw.h
  if (forceReload) {
    currentFramesPresetName.chop(QString(RELOAD_TAG).length());
    currentScene->getProperties()->setFramesPresetName(currentFramesPresetName);
  }
  if (currentFramesPresetName != framesName || forceReload ||
      currentScene != oldScene || fp != iniPath) {
    sizeList.clear();
